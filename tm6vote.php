<?php
	...
	if($submit)
	{
		...
		if($tm_pack) //this is a user voting
		{
			...
			if(!isSame($tm_1,$tm_2,$tm_3))
			{
				//first be sure they haven't voted before and are trying to scam us
				$query="SELECT tm_voted FROM tournamix_packs WHERE tm_pack=$tm_pack and tm=6";
				...
				if(!$flag)
				{
					...
					//6th place
					$query="SELECT tm_6 as total FROM tournamix WHERE tm_id=$tm_6";
					$result=mysql_db_query($database, $query);
					if($r=mysql_fetch_array($result))
					{
						extract($r);
						$total++;
						$result=mysql_db_query($database, "UPDATE tournamix SET tm_6=$total WHERE tm_id=$tm_6");
					}
					//7th place
					$query="SELECT tm_7 as total FROM tournamix WHERE tm_id=$tm_7";
					$result=mysql_db_query($database, $query);
					if($r=mysql_fetch_array($result))
					{
						extract($r);
						$total++;
						$result=mysql_db_query($database, "UPDATE tournamix SET tm_7=$total WHERE tm_id=$tm_7");
					}
					//8th place
					$query="SELECT tm_8 as total FROM tournamix WHERE tm_id=$tm_8";
					$result=mysql_db_query($database, $query);
					if($r=mysql_fetch_array($result))
					{
						extract($r);
						$total++;
						$result=mysql_db_query($database, "UPDATE tournamix SET tm_8=$total WHERE tm_id=$tm_8");
					}
					//9th place
					$query="SELECT tm_9 as total FROM tournamix WHERE tm_id=$tm_9";
					$result=mysql_db_query($database, $query);
					if($r=mysql_fetch_array($result))
					{
						extract($r);
						$total++;
						$result=mysql_db_query($database, "UPDATE tournamix SET tm_9=$total WHERE tm_id=$tm_9");
					}
					//10th place
					$query="SELECT tm_10 as total FROM tournamix WHERE tm_id=$tm_10";
					$result=mysql_db_query($database, $query);
					if($r=mysql_fetch_array($result))
					{
						extract($r);
						$total++;
						$result=mysql_db_query($database, "UPDATE tournamix SET tm_10=$total WHERE tm_id=$tm_10");
					}
					*/
					//register the vote
					$query="SELECT tm_voted, tm_votes FROM tournamix_packs WHERE tm_pack=$tm_pack and tm=6";
					$result=mysql_db_query($database, $query);
					if($r=mysql_fetch_array($result))
					{
						extract($r);
						...
						$tm_votes++;
						$result=mysql_db_query($database, "UPDATE tournamix_packs SET tm_voted='$tm_voted', tm_votes='$tm_votes' WHERE tm_pack=$tm_pack and tm=6");
					}
				
					path("Tournamix 6 Vote");
					start_table("TM 6: Vost cast!");
					echo "<tr><td align=center><b>You vote has been cast.<br><br>Results will be available soon!</td></tr>";
					end_table();
				}
				else
				{
					path("Tournamix 6 Vote");
					start_table("TM 6: wtf?!!");
	...
	else
	{
		if($results)
		{
			...
			$query="SELECT tm_done FROM tournamix_packs WHERE tm=6 AND tm_pack=$results";
			$result=mysql_db_query($database, $query);
			if($r=mysql_fetch_array($result))
				extract($r);	
			if($tm_done || (!$tm_done && $ddrei_level))
			{
				...
				path("<a href=tm6vote.php>Tournamix 6 Vote</a> :: Pack $results Results");
				...
				<td><b>10</b></td>
				<td><b>Total</b></td><tr>";
				$query="SELECT * FROM tournamix WHERE tm=6 AND tm_pack=$results";
				$result=mysql_db_query($database, $query);
				while($r=mysql_fetch_array($result))
				{
					...
					$t_dq[$i]=$tm_dq;
					$t_sum[$i]=$tm_1*$p1 + $tm_2*$p2 + $tm_3*$p3 + $tm_4*$p3 + $tm_5*$p3 + $tm_6*$p3 + $tm_7*$p3 + $tm_8*$p3 + $tm_9*$p3 + $tm_10*$p3;
					$i++;
		...
		else if(!$ddrei_id || !$ddrei_username || $ddrei_username=='ANONYMOUS' || $ddrei_regdate>1127758448)
		{
			path("Tournamix 6 Vote");
			...
			if($ddrei_regdate>1127758448)
				echo "<tr><td align=center>We're sorry, but to prevent users from creating accounts only to vote at the last minute, you must have registerred with the DDRei Forums <i>before</i> September 13, 2005.  You are not eligible to vote in this round.</td></tr>";
			
			$query="SELECT tm_pack FROM tournamix_packs WHERE tm_done=1 AND tm=6 Order by tm_pack desc limit 0,1";
			$result=mysql_db_query($database, $query);
			if($r=mysql_fetch_array($result))
				extract($r);
			if($tm_pack>1) 
			{
				...
				for($i=1; $i<=$tm_pack; $i++)
				{	
					echo "<a href=tm6vote.php?results=$i>Pack $i Results</a><br>";
				}
				echo "</td></tr>";
			}
		...
		{	
			...
			$query="SELECT tm_pack FROM tournamix_packs WHERE tm_active=1 AND tm=6";
			$result=mysql_db_query($database, $query);
			if($r=mysql_fetch_array($result))
				extract($r);
			$flag=0;
			if($tm_pack)
			{
				...
				//list all entries
				path("Tournamix 6 Vote - ");
				start_table("TM 6: Pack $tm_pack");
				if($flag)
				{
					echo "<tr><td align=center><br>'$ddrei_username' has already cast their vote for TM6, pack $tm_pack.<br><br>Please return in a week for the results and the next vote!</td></tr>";
				}
				else
				{
					echo "<tr><td colspan=2 align=center>Please select your top 3 step charts from TM6, pack $tm_pack.</td></tr>
					
					<tr><td align=center><b>1)</b></td><td valign=bottom align=center>";
					
					echo "<form method=POST action=tm6vote.php><select size=1 name=tm_1><option value=\"\">------------</option>";
					$query="SELECT tm_id, tm_song, tm_creator FROM tournamix WHERE tm_pack=$tm_pack AND tm=6 order by tm_song";
					$result=@mysql_db_query($database, $query);
					while($r=@mysql_fetch_array($result))
					{
						extract($r);
